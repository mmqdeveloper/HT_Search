	...
	if ( $wishlist && $wishlist->has_items() ) :
		foreach ( $wishlist_items as $item ) :
			...
			if ( $product && $product->exists() ) :
				?>
				<li id="yith-wcwl-row-<?php echo esc_attr( $item->get_product_id() ); ?>" data-row-id="<?php echo esc_attr( $item->get_product_id() ); ?>" class="product type-product">
					<a href="<?php echo esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', $item->get_product_id() ) ) ); ?>" class="woocommerce-LoopProduct-link woocommerce-loop-product__link">
						...
						<!--</a>-->
						<?php 
							$mm_builder_open = get_post_meta( $item->get_product_id(), 'mm_builder', true );
									...
									<?php
										...
										if($number_list_items > 0){
											...
											for( $i = 0; $i < $number_list_items; $i++ ){
												$list_items_text = get_post_meta( $post_id, 'short_description_list_items_' . $i . '_text', true );
												$list_items_icon = get_post_meta( $post_id, 'short_description_list_items_' . $i . '_icon', true );
												if( $list_items_text ){
						...
						<div class="inner_product_header">
							<?php
							$excerpt = get_the_excerpt($item->get_product_id());
