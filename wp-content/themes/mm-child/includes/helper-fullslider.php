...
if ( !class_exists( 'avia_fullslider' ) )
{
	class avia_fullslider
	{
		...
		public function html()
		{
			...
			$html .= '<div>';
			$html .= '<div style="    width: 45%;float: left;position: absolute;"><img src="https://redlinerafting.com/dev/wp-content/uploads/image-fullsld-home.png" /></div>';
			$html .= "<div {$data} style='width: 55%;float: right;' class='avia-slideshow avia-slideshow-".avia_fullslider::$slider." {$extraClass} avia-slideshow-".$this->config['size']." ".$this->config['handle']." ".$this->config['class']." avia-".$this->config['animation']."-slider ' $markup>";
		...
		protected function advanced_slide()
		{
			...
			foreach($this->id_array as $key => $id)
			{
				...
				if(isset($this->slides[$id]) || $slide_type == 'video')
				{
					...
					$html .= "<".$tags[0]." data-rel='slideshow-".avia_fullslider::$slider."' class='avia-slide-wrap' {$linkdescription} >{$caption}";
