 ...
 */
if (!class_exists('avia_sc_tab_mm')) {

    class avia_sc_tab_mm extends aviaShortcodeTemplate {
        ...
        function shortcode_insert_button() {
            $this->config['name'] = __('MM Tabs', 'avia_framework');
            ...
            $this->config['shortcode'] = 'av_tab_container_mm';
            $this->config['shortcode_nested'] = array('av_tab_mm');
        ...
        function popup_elements() {
            $this->elements = array(
                ...
                array(
                    "name" => __("Title Tab", 'avia_framework'),
                    "desc" => __("Enter the title.", 'avia_framework'),
                    "id" => "title",
                    "std" => "",
                    "type" => "input"),
                array(
        ...
        function shortcode_handler($atts, $content = "", $shortcodename = "", $meta = "") {
            ...
            $atts = shortcode_atts(array(
                ...
                'title' => '',
                'position' => 'top_tab',
                'boxed' => 'border_tabs'), $atts, $this->config['shortcode']);
            extract($atts);
            ...
            avia_sc_tab_mm::$counter = 1;
            avia_sc_tab_mm::$title = $title;
            avia_sc_tab_mm::$initial = $initial;
        ...
        function av_tab_mm($atts, $content = "", $shortcodename = "") {

            $output = $titleClass = $contentClass = $icon = "";
            $tab_atts = shortcode_atts(array('title' => '', 'icon_select' => 'no', 'icon' => "", 'custom_id' => '', 'font' => '', 'custom_markup' => ''), $atts, 'av_tab_mm');

            $display_char = av_icon($tab_atts['icon'], $tab_atts['font']);

            if (is_numeric(avia_sc_tab_mm::$initial) && avia_sc_tab_mm::$counter == avia_sc_tab_mm::$initial) {
            ...
            if (empty($tab_atts['custom_id'])) {
                $tab_atts['custom_id'] = 'tab-id-' . avia_sc_tab_mm::$tab_id++;
            }
            ...
            $output .= '<section class="av_tab_section mm-tab ssss " ' . $markup_tab . '>';
            $output .= '    <div  class="tab "' . $markup_title . '>Aloha</div>' . "\n";
