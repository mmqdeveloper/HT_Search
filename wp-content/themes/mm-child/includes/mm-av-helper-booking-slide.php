...
if ( !class_exists( 'mm_avia_slideshow' ) )
{
	class mm_avia_slideshow
	{
		...
		protected function advanced_slide()
		{
			...
			foreach($this->id_array as $key => $id)
			{
				...
				if(isset($this->slides[$id]) || $slide_type == 'video')
				{
					...
					if(trim($title) != "")   $title 	= "<h2 {$title_styling} class='mm-avia-caption-title' $markup_name>".trim(apply_filters('avf_slideshow_title', $title))."</h2>";
					...
					if(trim($content) != "") $content 	= "<div class='mm-avia-caption-content {$content_class}' {$markup_description} {$content_styling}>".ShortcodeHelper::avia_apply_autop(ShortcodeHelper::avia_remove_autop('$'.trim($content)))."</div>";
					...
					if(trim($title.$content.$button_html) != "")
					{
						...
						{
							$caption = '<div class="mm-avia-caption av-slideshow-caption"><div class="avia-inner-caption">'.$title.$content."</div></div>";
						}
					}
					...
					{
						$html .= "<img style='width:267px;' src='".$img[0]."' width='".$img[1]."' height='".$img[2]."' title='".$linktitle."' alt='".$linkalt."' $markup_url />";
					}
...
if ( !class_exists( 'mm_avia_slideshow_video_helper' ) )
{
	class mm_avia_slideshow_video_helper
	{
