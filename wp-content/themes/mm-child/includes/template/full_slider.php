<?php 
if ( !class_exists( 'red_avia_slideshow' ) )
{
	class red_avia_slideshow
	{
		...
		public function html()
		{
			...
			$class_add_slider = '';
			if(!empty($this->config['cb_show_slider'])){
				$class_add_slider = 'red-slider-tabs container_'.$this->config['input_key'];
			} 
			$html .='<div class="red-btn">';
			$html .='<div class="container">';	
				$html .= '<div>';
					if(!empty($this->config['header_slider_1']) && isset($this->config['header_slider_1'])){ $html .= '<button type="button" data-slider="'.$this->config['input_key'].'" type_sl="main" class="btn_slider active_btn">'.($this->config['header_slider_1']).'</button>';}
					if(!empty($this->config['header_slider_2']) && isset($this->config['header_slider_2'])){ $html .= '<button type="button" data-slider="'.$this->config['input_key'].'" type_sl="child" class="btn_slider">'.($this->config['header_slider_2']).'</button>';}
				$html .= '</div>';	
				$html .= '<div style="margin-top: 15px;">';
					if(!empty($this->config['header_slider_3']) && isset($this->config['header_slider_3'])){ $html .= '<button type="button" data-slider="'.$this->config['input_key3'].'" type_sl="child2" class="btn_slider">'.($this->config['header_slider_3']).'</button>';}
				$html .= '</div>';	
			$html .='</div>';
			$html .='</div>'; 	
			$html .= "<div {$data} class='avia-slideshow ".$class_add_slider." red-slideshow red-slideshow-".red_avia_slideshow::$slider." avia-slideshow-".red_avia_slideshow::$slider." {$extraClass} avia-slideshow-".$this->config['size']." ".$this->config['handle']." ".$this->config['class']." avia-".$this->config['animation']."-slider ' $markup>";
			...
			$html .= "</div>";
			$html .='<style>
				.red-slideshow-'.red_avia_slideshow::$slider.'{
					background:url("'.$this->config['src_image'].'");
		...
		protected function advanced_slide()
		{
			...
			foreach($this->id_array as $key => $id)
			{
				...
				if(isset($this->slides[$id]) || $slide_type == 'video')
				{
					...
					if($slide_type == 'video')
					{
						$this->service    = red_avia_slideshow_video_helper::which_video_service($video);
						$video 			  = red_avia_slideshow_video_helper::set_video_slide($video, $this->service, $meta); 
					...
					if(trim($title.$content.$button_html) != "")
					{
						...
						{
							$caption = '<div class="avia-caption av-slideshow-caption container"><div class="avia-inner-caption">'.$title.$content."</div></div>";
						}
					}
					...
					$html .= "<".$tags[0]." data-rel='slideshow-".red_avia_slideshow::$slider."' class='avia-slide-wrap' {$linkdescription} >{$caption}";
		...
		{ 
			...
			$html .= 	"<a href='#prev' class='prev-slide prev-btn' ".av_icon_string('prev_big').">".__('Previous','avia_framework' )."</a>";
			$html .= 	"<a href='#next' class='next-slide next-btn active_slider' ".av_icon_string('next_big').">".__('Next','avia_framework' )."</a>";
			$html .= "</div>";  
...
if ( !class_exists( 'red_avia_slideshow_video_helper' ) )
{
	class red_avia_slideshow_video_helper
	{
