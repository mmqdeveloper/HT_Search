...
if ( !class_exists( 'red_avia_slideshow_screen' ) )
{
	class red_avia_slideshow_screen
	{
		...
		public function html()
		{
			...
			$style   	= "";
			$extraClass = "";
			red_avia_slideshow_screen::$slider++;
			...
			$html .= "<div {$data} class='red-slideshow avia-slideshow-".red_avia_slideshow_screen::$slider." {$extraClass} avia-slideshow-".$this->config['size']." ".$this->config['handle']." ".$this->config['class']." avia-".$this->config['animation']."-slider ' $markup>";
			
			
			$html .= "<ul class='red-slideshow-inner avia-slideshow-inner' {$style}>";
		...
		protected function advanced_slide()
		{
			...
			foreach($this->id_array as $key => $id)
			{
				...
				if(isset($this->slides[$id]) || $slide_type == 'video')
				{
					...
					if($slide_type == 'video')
					{
						$this->service    = red_avia_slideshow_screen_video_helper::which_video_service($video);
						$video 			  = red_avia_slideshow_screen_video_helper::set_video_slide($video, $this->service, $meta); 
					...
					if(trim($title) != "")   $title 	= "<div {$title_styling} class='avia-caption-title' $markup_name>".trim(apply_filters('avf_slideshow_title', $title))."</div>";
					...
					$html .= "<".$tags[0]." data-rel='slideshow-".red_avia_slideshow_screen::$slider."' class='avia-slide-wrap red-fullscreen".red_avia_slideshow_screen::$slider."' style='background-image:url(".$img[0]."); background-size: cover; background-repeat: no-repeat;background-position: 50% 0%;' {$linkdescription} >{$caption}"; //style='background-image:url(".$img[0]."); background-size: cover; background-repeat: no-repeat;background-position: 50% 0%;'
					if($this->config['bg_slider'] != "true" && empty($video))
					{
						$html .= "<img src='".$img[0]."' width='".$img[1]."' height='".$img[2]."' title='".$linktitle."' alt='".$linkalt."' $markup_url style='display:none;'/> ";
					}
					$html .= $video;
		...
		{
			$html   = ""; 
			$html  .= "<div class='avia-slideshow-dots ".(($this->config['autoplay'] == 'true')?'auto-slideshow-dots':'')." avia-slideshow-controls'>";
			$active = "active";

			for($i = 1; $i <= $this->slide_count; $i++)
			{
				$html .= "<a href='#{$i}' ids='{$i}' class='goto-slide {$active}' >{$i}</a>";
				$active = "";
			}
...
if ( !class_exists( 'red_avia_slideshow_screen_video_helper' ) )
{
	class red_avia_slideshow_screen_video_helper
	{
