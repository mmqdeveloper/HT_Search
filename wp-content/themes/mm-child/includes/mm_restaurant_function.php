...
if (!function_exists('mm_search_restaurant_with_keyword')) {
    function mm_search_restaurant_with_keyword ($keyword) {
        global $wpdb;
        $query = $wpdb->prepare("
            SELECT ID
            ...
            AND (
                ...
                OR (tt.taxonomy = 'restaurant_categories' AND LOWER(t.name) LIKE '%%%s%%')
                OR (tt.taxonomy = 'restaurant_island' AND LOWER(t.name) LIKE '%%%s%%')
                OR (tt.taxonomy = 'restaurant_tags' AND LOWER(t.name) LIKE '%%%s%%')
            )
...
if (!function_exists('mm_restaurant_get_search_absolute_results')) {
    function mm_restaurant_get_search_absolute_results ($array_in) {
...
add_action('wp_ajax_mm_ajax_filter_restaurant', 'mm_ajax_filter_restaurant');
add_action('wp_ajax_nopriv_mm_ajax_filter_restaurant', 'mm_ajax_filter_restaurant');
if (!function_exists('mm_ajax_filter_restaurant')) {

    function mm_ajax_filter_restaurant() {
        global $wpdb;
        ...
        if (!empty($search)) {
            ...
            if (strpos($search, ' ') !== false) {
                ...
                foreach($key_separation as $key) {
                    $result_list[] = mm_search_restaurant_with_keyword($key);
                }
                $post_ids = mm_restaurant_get_search_absolute_results($result_list);
            } else {
                $post_ids = mm_search_restaurant_with_keyword($search);
            }
        ...
        if (have_posts()) {
            ...
            echo __('<p style="padding:150px 0;text-align:center;font-size:18px;">No restaurants found</p>');
        }

        $output_pagination = restaurant_pagination( '', 'nav' );
...
add_action('wp_ajax_mm_ajax_pagination_restaurant', 'mm_ajax_pagination_restaurant');
add_action('wp_ajax_nopriv_mm_ajax_pagination_restaurant', 'mm_ajax_pagination_restaurant');
if (!function_exists('mm_ajax_pagination_restaurant')) {

    function mm_ajax_pagination_restaurant() {
        global $wpdb;
...
if (!function_exists('restaurant_pagination')) {
    ...
    function restaurant_pagination($pages = '', $wrapper = 'div', $query_arg = '', $current_page = 1) {
        ...
        if (1 != $pages) {
            $output .= "<{$wrapper} class='pagination mm-pagination-restaurant' data-island='all'>";
