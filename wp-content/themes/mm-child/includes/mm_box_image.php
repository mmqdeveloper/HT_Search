 ...
 * 
 */
if (!class_exists('avia_sc_box_image')) {

    class avia_sc_box_image extends aviaShortcodeTemplate {

        /**
         * Create the config array for the shortcode button
         */
        function shortcode_insert_button() {
            $this->config['name'] = __('MM Box Image', 'avia_framework');
            ...
            $this->config['shortcode'] = 'av_box_image';
            //$this->config['modal_data']     = array('modal_class' => 'mediumscreen');
            $this->config['tooltip'] = __('Create a box image', 'avia_framework');
        }
        ...
        function popup_elements() {
            $this->elements = array(
                ...
                array(
                    ...
                    "desc" => __("Text display above image", 'avia_framework'),
                    "id" => "text_image",
                    "type" => "input",
                    "std" => "",
                ),
        ...
        function shortcode_handler($atts, $content = "", $shortcodename = "", $meta = "") {
            ...
            if (!empty($src)) {
                ...
                if (is_numeric($src)) {
                    ...
                    if ($detect->isMobile()) {
                        $link = (!empty($link)) ? aviaHelper::get_url($link, $attachment) : '';
                    } else {
                        if (empty($link)) {
                            $link = "lightbox";
                        }
                    ...
                    if ($caption == "yes") {
                        ...
                        $overlay = "{$overlay_bg}<div class='mm-av-image-caption-overlay-position'><div class='av-image-caption-overlay-center' {$style}>{$content}</div></div>";
                        $class .= " noHover ";
                    ...
                    if ($text_image) {
                        $text_image = str_replace('"', "'", $text_image);
                    }
                    ...
                    $output .= "<div class='mm-box-image avia-image-container {$class} " . $meta['el_class'] . " " . $this->class_by_arguments('align', $atts, true) . "' $markup >";
                    $output .= "<div class='mm-avia-image-container-inner'>";
                    if (!empty($link)) {
                        $output .= "<a href='{$link}' class='avia_image'  {$blank}>{$overlay}<div class='text_image_mm'>{$text_image}</div><img class='mm-avia_image ' src='{$src}' alt='{$alt}' title='{$title}' $markup_url /></a>";
                    } else {
                        $output .= "{$overlay}<img class='mm-avia_image' src='{$src}' alt='{$alt}' title='{$title}'  $markup_url />";
                    }
                    $output .= "</div>";
                    $output .= "</div>";
                }
