	...
	class avia_sc_gallery extends aviaShortcodeTemplate
	{
			...
			function popup_elements()
			{
				$this->elements = array(
					...
					array(
							...
							"std" 	=> "thumbnails",
							"subtype" => array(
												__('Images only',  'avia_framework' ) =>'images_only',
			...
			function shortcode_handler($atts, $content = "", $shortcodename = "", $meta = "")
			{
				...
				if(!empty($attachments) && is_array($attachments))
				{
					...
					$output .= "<div class='avia-gallery avia-mm-gallery avia-gallery-".self::$gallery." ".$lazyload." avia_animate_when_visible ".$meta['el_class']."' $markup>";
					$thumbs = "";
					$counter = 0;
					...
					foreach($attachments as $attachment)
					{
						...
						if( strpos($style, "images_only") !== false && empty($size)) {
			   ...
			   if($caption) $output .= "	<span class='avia-gallery-caption'>{$caption}</span>";
						...
						$thumbs .= " <a ben='test' href='".$link[0]."' data-rel='gallery-".self::$gallery."' data-prev-img='".$prev[0]."' {$class} data-onclick='{$counter}' title='".$description."' $markup_url><img {$tooltip} src='".$img[0]."' width='".$img[1]."' height='".$img[2]."'  title='".$title."' alt='".$alt."' /></a>";
						$first = false;
					}

					$output .= "<div class='avia-gallery-thumb ".(strpos($style, "images_only") !== false ? 'images-only' : '')."'>{$thumbs}</div>";
					$output .= "</div>";
