	...
	class avia_sc_tab_section extends aviaShortcodeTemplate
	{
		...
		protected function register_dynamic_templates()
		{
			...
			$c = array(
						...
						array(
							'name'		=> __( 'Show arrow slider', 'avia_framework' ),
							'id'		=> 'show_arrow_slider',
							'desc'		=> __( 'Show arrow slider for tabs on small screen.', 'avia_framework' ),
							'type'		=> 'select',
							'std'		=> '',
							'subtype'	=> array(  	
			...
			$c = array(
						...
						array(
							...
							'desc'  => __( 'Here you can set the text color of the tab active. Enter no value if you want to use the standard font color.', 'avia_framework' ),
							'id'    => 'tab_active_color',
							'type'  => 'colorpicker',
							'rgba' 	=> true,
							'std' 	=> ''
		...
		function shortcode_handler( $atts, $content = '', $shortcodename = '', $meta = '' )
		{
			...
			for( $i = 1; $i <= avia_sc_tab_section::$tab; $i ++ )
			{
				...
				$tabs .= "<a href='#{$tab_link}' data-av-tab-section-title='{$i}' class='av-section-tab-title {$active_tab} {$extraClass} ' role='tab' tabindex='0' aria-controls='{$tab_id}'><div class='row-img'>{$icon}{$image}<span class='av-outer-tab-title'><span class='av-inner-tab-title'>{$tab_title}</span></span>{$arrow}</div></a>";
			}
			...
			if( ! empty( $atts['tab_active_color'] ) && ! empty( $params['id'] ) )
			{
				$tab_active_style .= AviaHelper::style_string( $atts, 'tab_active_color', 'color' );
				echo "<style>#".$params['id']." .av-tab-section-tab-title-container > a.av-active-tab-title {".$tab_active_style."}</style>";
			}

			if( $atts['show_arrow_slider'] == 'yes' )
			{
				$show_arrow_slider = 'tab-ss-show-arrow-slider';
			} else {
				$show_arrow_slider = '';
			}
			...
			$output .=  "<div class='av-tab-section-outer-container {$show_arrow_slider}'>";
