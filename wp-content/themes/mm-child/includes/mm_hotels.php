...
if (!class_exists('mm_list_hotels')) {

    class mm_list_hotels extends aviaShortcodeTemplate {
        ...
        function shortcode_insert_button() {
            ...
            $this->config['name'] = __('List Hotels', 'avia_framework');
            ...
            $this->config['shortcode'] = 'mm_hotels';
            $this->config['tooltip'] = __('Displays list Hotels', 'avia_framework');
        ...
        protected function register_dynamic_templates() {
            ...
            $c = array(
                array(
                    'name' => __('Hotels Style', 'avia_framework'),
                    'desc' => __('Choose the default Hotels layout here.', 'avia_framework'),
                    'id' => 'blog_style',
                    'type' => 'select',
                    'std' => 'single-big',
                    'lockable' => true,
...
//												__( 'No Sidebar', 'avia_framework' )	=> 'fullsize'
                ...
                array(
                    'name' => __('Define Hotels Grid layout', 'avia_framework'),
            ...
            $c = array(
                array(
                    'name' => __('Hotels List Width', 'avia_framework'),
                ...
                array(
                    'name' => __('Hotels Grid Columns', 'avia_framework'),
        ...
        function shortcode_handler($atts, $content = '', $shortcodename = '', $meta = '') {
            ...
            if ($atts['blog_style'] == 'blog-grid') {
                ...
                $slider = new mm_hotel_slider($atts);
            ...
            if (!empty($atts['show_siderbar'])) {
                ...
                {
                    echo "<div class='{$atts['blog_style']} restaurant-pagination'>" . hotel_pagination( '', 'nav' ) . '</div>';
                }
                if (!empty($atts['show_siderbar'])) {
                    ?>
                </div>
                <div class="flex_column av_one_third  flex_column_div av-zero-column-padding sidebar-restaurant  sidebar-hotel" style="border-radius:0px; ">
                    ...
                    <?php
                    $hotel_island = get_terms(array(
                        'taxonomy' => 'hotel_island',
                        'hide_empty' => true
                    ));

                    if (!empty($hotel_island)) :
                        echo "<h3>Island</h3>";
                        ...
                        foreach ($hotel_island as $island) {
                    ...
                    ?>
                    <?php
                    $hotel_categories_parent = get_terms(array(
                        'taxonomy' => 'hotel_categories',
                        'hide_empty' => true,
                        'parent'     => 0,
                    ));

                    if (!empty($hotel_categories_parent)) :
                        ...
                        foreach ($hotel_categories_parent as $categories_parent) {
                            $hotel_categories_child = get_terms(array(
                                'taxonomy' => 'hotel_categories',
                                'hide_empty' => true,
                            ...
                            if (!empty($hotel_categories_child)) {
                                ...
                                foreach ($hotel_categories_child as $category_child) {
                        ...
                        // if (count($hotel_categories_parent) > 4) {
                        //     echo '<span class="list-restaurant-term-more-btn">More</span>';
                        // }
                        echo "</ul>";
                    endif;
                    ...
                    <?php
                        ...
                        //         echo "<li class='item_cate' data-id='" . $tag->term_id . "'>" . esc_html($tag->name) . "</li>";
                        //     }
                        //     if (count($hotel_tags) > 4) {
                        //         echo '<span class="list-restaurant-term-more-btn">More</span>';
                        //     }
                        //     echo "</ul>";
                        // endif;
        ...
        public function handler_pagination_link_method($method, $pages, $wrapper) {
            if (is_single() || ( 'get_pagenum_link' == $method )) {
                $method = 'mm_list_hotels::add_blog_pageing';
            }
...
if (!class_exists('mm_hotel_slider')) {

    class mm_hotel_slider {
        ...
        public function html() {
            ...
            $output .= "<div id='mm-list-restaurants' {$data} class='avia-content-slider avia-content-{$type}-active avia-content-slider" . mm_hotel_slider::$slide . " avia-content-slider-{$total} {$class}' $markup>";
            ...
            foreach ($this->entries->posts as $entry) {
                ...
                $output .= $thumbnail ? "<a href='{$link}' data-rel='slide-" . mm_hotel_slider::$slide . "' class='slide-image' title=''>{$thumbnail}</a>" : "";
            ...
            if ($use_main_query_pagination == 'yes' && $paginate == "yes") {
                global $wp_query;
                $avia_pagination = hotel_pagination($wp_query->max_num_pages, 'nav');
            } else if ($paginate == "yes") {
                $avia_pagination = hotel_pagination($this->entries->max_num_pages, 'nav');
            }
...
// show hotel list in badges
if (!function_exists('mm_show_list_hotel_in_badges')) {
    function mm_show_list_hotel_in_badges ($slug_badges) {
        $args_hotel_badges = array(
            'post_type' => 'hotel',
            'posts_per_page' => -1,
            'post_status' => 'publish',
            'tax_query' => array(
        ...
        $hotels_with_badges = get_posts($args_hotel_badges);
        if (!empty($hotels_with_badges)) {
        ?>
        <div class="mm-hotel-badges-wrap">
            <h3>Hotels</h3>
            <div class="mm-hotel-badges-wrap-inner">
            <?php
            foreach($hotels_with_badges as $hotel) {
                $the_id = $hotel->ID;
    ...
    add_action('mm_show_list_hotel_badges_product', 'mm_show_list_hotel_in_badges', 10, 1);
}