...
if (!class_exists('mm_avia_sc_section')) {

    class mm_avia_sc_section extends aviaShortcodeTemplate {
        ...
        function shortcode_handler($atts, $content = "", $shortcodename = "", $meta = "") {
            ...
            mm_avia_sc_section::$section_count++;
            ...
            extract($atts);
            $output = "";
            $class = "mm-avia-section-content " . $color . " avia-section-content-" . $padding . " avia-" . $shadow;
            $background = "";


            $params['id'] = !empty($id) ? AviaHelper::save_string($id, '-') : "mm_av_section_content_" . mm_avia_sc_section::$section_count;
                ...
                $background .= "background-position: {$position}; ";
                 if (!empty($min_height)) {
                    ...
                    if ($min_height == 'custom' && $min_height_px != "") {
                       ...
                       $background .= "min-height: {$min_height_px}px; ";
                    }
                }
            ...
            $output .= mm_avia_new_section($params);
            ...
            if (strpos($bottom_border, 'border-extra') !== false) {
                ...
                mm_avia_sc_section::$add_to_closing = "<div class='av-extra-border-element {$bottom_border}'><div class='av-extra-border-outer'><div class='av-extra-border-inner' {$backgroundEl}></div></div></div>";
            }
            else {
                mm_avia_sc_section::$add_to_closing = "";
            }


            //next section needs an extra closing tag if overlay with wrapper was added:
            if ($overlay || !empty($scroll_down)) {
                mm_avia_sc_section::$close_overlay = "</div>";
            } else {
                mm_avia_sc_section::$close_overlay = "";
            }
            ...
            if (empty($skipSecond)) {
                $new_params['id'] = "after_section_" . ( mm_avia_sc_section::$section_count );
               // $output .= mm_avia_new_section($new_params);
            }
            $output .='</div>';
...
if (!function_exists('mm_avia_new_section')) {

    function mm_avia_new_section($params = array()) {
        ...
        if ($open) {
            ...
            if ($open_color_wrap) {
                ...
                $output .= "<div {$id} class='{$class} container_wrap mm_section_element_container" . avia_layout_class('main', false) . "' {$bg} {$data} {$style}>";
                $output .=!empty($bg_slider) ? $bg_slider->html() : "";
                $output .= $attach;
                $output .= apply_filters('mm_avf_section_container_add', '', $defaults);
            }
