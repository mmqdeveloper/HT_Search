...
if (!class_exists('mm_custom_sc_image')) {

    class mm_custom_sc_image extends aviaShortcodeTemplate {
        ...
        function shortcode_insert_button() {
            ...
            $this->config['name'] = __('Image Desktop / Image Mobile', 'avia_framework');
            ...
            $this->config['shortcode'] = 'mm_image';
            //$this->config['modal_data']     = array('modal_class' => 'mediumscreen');
            $this->config['tooltip'] = __('Inserts an image for desktop or mobile of your choice', 'avia_framework');
        ...
        function popup_elements() {
            $this->elements = array(
                ...
                array(
                    "name" => __("Choose Image Desktop", 'avia_framework'),
        ...
        function shortcode_handler($atts, $content = "", $shortcodename = "", $meta = "") {
            ...
            if (!empty($attachment)) {
                ...
                $attachment_size = explode(',', $attachment_size);
                $attachment_entry = get_post($attachment[0]);
                $attachment_entry_mobile = get_post($attachment[1]);
                if (!empty($attachment_entry)) {
                    ...
                    if (!empty($attachment_size[0])) {
                        $src = wp_get_attachment_image_src($attachment_entry->ID, $attachment_size[0]);
                ...
                if (!empty($attachment_entry_mobile)) {
                    if (!empty($attachment_size[1])) {
                        $src_mobile = wp_get_attachment_image_src($attachment_entry_mobile->ID, $attachment_size[1]);

                        $img_h_mobile = !empty($src_mobile[2]) ? $src_mobile[2] : "";
                        $img_w_mobile = !empty($src_mobile[1]) ? $src_mobile[1] : "";
            ...
            if (!empty($src)) {
                ...
                if (is_numeric($src)) {
                    ...
                    if ($link) {
                        $output .= "<a href='{$link}' class='avia_image'  {$blank}>{$overlay}<img class='avia_image show_image_desktop' src='{$src}' alt='{$alt}' title='{$title}' $markup_url />";
                        if($src_mobile!==''){
                            $output .= "{$overlay}<img class='avia_image show_image_mobile' src='{$src_mobile}' alt='{$alt}' title='{$title}' $markup_url />";
                        }
                        $output .= "</a>";
                    } else {
                        $hw = "";
                        ...
                        $output .= "{$overlay}<img class='avia_image show_image_desktop' src='{$src}' alt='{$alt}' title='{$title}' {$hw} $markup_url />";
                        if($src_mobile!==''){
                            $hw_mobile = "";
                            if (!empty($img_h_mobile))
                                $hw_mobile .= 'height="' . $img_h_mobile . '"';
                            if (!empty($img_w_mobile))
                                $hw_mobile .= 'width="' . $img_w_mobile . '"';
                            $output .= "{$overlay}<img class='avia_image show_image_mobile' src='{$src_mobile}' alt='{$alt}' title='{$title}' {$hw_mobile} $markup_url />";
                        }
                    }
                    $output .= "</div>";
