 ...
 */
if (!class_exists('avia_sc_mm_image_hotspots')) {

    class avia_sc_mm_image_hotspots extends aviaShortcodeTemplate {

        /**
         * Create the config array for the shortcode button
         */
        function shortcode_insert_button() {
            $this->config['name'] = __('MM Image with Hotspots', 'avia_framework');
            ...
            $this->config['shortcode'] = 'av_mm_image_hotspot';
        ...
        function popup_elements() {
            $this->elements = array(
                ...
                array(
                    "name" => __("Show popup", 'avia_framework'),
                    "desc" => __("Check to only display popup", 'avia_framework'),
                    "id" => "check_popup",
                    "type" => "checkbox",
                    "std" => "",
                ),
                array(
                    "name" => __("Content Popup", 'avia_framework'),
                    "desc" => __("The ID gravityform", 'avia_framework'),
                    "id" => "content_popup",
                    "type" => "input",
                    "required" => array('check_popup', 'equals', 'aviaTBcheck_popup'),
                ),
                array(
        ...
        function shortcode_handler($atts, $content = "", $shortcodename = "", $meta = "") {
            ...
            extract(shortcode_atts(array('animation' => 'no-animation', 'attachment' => '', 'attachment_size' => '', 'hotspot_layout' => 'numbered', 'hotspot_mobile' => '', 'hotspot_tooltip_display' => '', 'check_popup' => '', 'content_popup' => ''), $atts, $this->config['shortcode']));
            if (!empty($attachment)) {
            ...
            if (!empty($src)) {
                ...
                $output .= $hotspot_html;
                $output .= '<a href="https://fareharbor.com/embeds/book/himermaid/items/33188/calendar/" onclick="return !(window.FH && FH.open({ shortname: \'himermaid\', fallback: \'simple\', view: { item: 33188 } }));"  >';

                $output .= "<img class='avia_image' src='{$src}' alt='{$alt}' title='{$title}'  $markup_url /></a>";
                $output .= "</div>";
                $output .= "</div>";
                $output .= "</div>";
                $output .= $tooltip_html;
                ...
                if (!empty($check_popup)) {
                    $output .= '<div id="image_hotposts_popup" class="mfp-hide white-popup-block">';
                    $output .= do_shortcode('[gravityform id=' . $content_popup . ' title=false description=false ajax=true]');
                    $output .= '</div>';
                }
            }
        ...
        function add_hotspot($hotspot, $counter, $extraClass = "") {
            extract(shortcode_atts(array('tooltip_width' => 'av-tt-default-width', 'tooltip_pos' => 'av-tt-pos-above av-tt-align-left', 'hotspot_pos' => '50,50', 'output' => '', 'hotspot_color' => '', 'custom_bg' => '', 'custom_font' => '', 'custom_pulse' => '', 'tooltip_style' => 'main_color', 'link' => '', 'link_target' => '', 'check_popup' => ''), $hotspot['attr']));
